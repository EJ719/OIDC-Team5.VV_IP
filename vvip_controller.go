...
func (r *VvipReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	reqLogger.Info("Reconciling Vvip.")
	...
	// Update the Vvip status with the pod names
	// List the pods for this vvip's deployment
...
// serviceForVvip function takes in a Vvip object and returns a Service for that object.
func (r *VvipReconciler) serviceForVvip(v *vvipv1.Vvip) *corev1.Service {
	ls := labelsForVvip(v.Name)
	ser := &corev1.Service{
		ObjectMeta: v1.ObjectMeta{
			Name:      v.Name,
	...
	ctrl.SetControllerReference(v, ser, r.Scheme)
	return ser
}
